function STDPvsPDDP(N,omega,Delta,theta_0_std,kappa_0,kappa_0_std,tau_plus,tau_R,beta,A_plus,n_modes,outputDir,seed,timeInS,dt)
...
dirStr = [outputDir filesep 'STDPvsPDDP_' parStr '_' getDateTimeStr];
mkdir(dirStr);
...
[~,r_pop_pddp,k_avg_pddp,omegaVect_pddp,snap_pddp] = kuramoto_onePop_FourierPDDP_fwdSim(par,simPar);
...
plot(t,angle(squeeze(r_pop_pddp)),'linewidth',2*l_osc,'displayName',['PDDP, ' num2str(n_modes) ' Fourier modes'])
...
mainTitleStr = {['$\Omega = $ ' num2str(par.omega,2) ', $\Delta = $ ' num2str(par.Delta,2) ...
    ...
    ', $\tau_- = $ ' num2str(tau_R*tau_plus)], ['PDDP with ' num2str(n_modes) ' Fourier modes']};
...
plot(t,rho_pddp,'linewidth',2*l_osc,'displayName',['PDDP, ' num2str(n_modes) ' Fourier modes'])
ylim([0 1])
legend('location','best')
...
plot(t,k_avg_pddp_sq,'linewidth',2*l_osc,'displayName','PDDP')
...
plotWeightDistrOverTime(snap_pddp,k_avg_pddp,t,simPar,par,N,dirStr,['PDDP, ' num2str(n_modes) ' Fourier modes'],ft_k,false,[],[],[]);
...
[~,fnames{6}] = plotWeightDistrOverTime(snap_pddp,k_avg_pddp,t,simPar,par,N,dirStr,['PDDP, ' num2str(n_modes) ' Fourier modes'],ft_k,true,c_max_stdp,k_min_stdp,k_max_stdp);
...
fnames{7} = plotCouplingMat(snap_pddp,N,n_t_plot,sortType,omegaVect_pddp,[],dirStr,['PDDP, ' num2str(n_modes) ' Fourier modes'],ft_k,[k_min_stdp k_max_stdp]);
