function [theta,r_pop,k_avg,omegaVect,snap,k0] = kuramoto_onePop_FourierEbPDDP_fwdSim(par,simPar)
...
for i_t = 1:n-1
    ...
    deltaMat = (zeroXing_bool + zeroXing_bool')>0;
    ...
    if any(deltaMat,'all')
        ...
        phi = phi';%pre,post
        k = k + pi/dt/omega*reshape(deltaMat,[1,N,1,N]).*reshape(par.f(phi),[1,N,1,N])*dt;%tested OK
    end
